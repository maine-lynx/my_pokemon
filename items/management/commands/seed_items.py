...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in items_data:
            obj, created = Item.objects.get_or_create(item_id=data["item_id"], defaults=data)
        ...
        for trainer in trainers:
            for iid, qty in [(1, 5), (2, 2), (3, 10), (4, 3)]:
                TrainerItem.objects.get_or_create(trainer=trainer, item=item_objs[iid], defaults={"quantity": qty})
            self.stdout.write(self.style.SUCCESS(f"✅ {trainer.name} 获得初始道具"))
